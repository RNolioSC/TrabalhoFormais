...
class View:
    ...
    def change_window(self):
        ...
        if self.operacao == 8 or self.operacao == 9:
                self.sentence.grid(row=0, column=1)
    ...
    def formata_resultado_gr(self):
        ...
        # GENERALIZAR ESSA PARTE - CODIGO REDUNDANTE
        tmp = estado_inicial + "->"

        for list in range(len(self.result[estado_inicial])):
            tmp += self.result[estado_inicial][list]
            if list+1 < len(self.result[estado_inicial]):
                if not self.result[estado_inicial][list+1].istitle():
                    tmp += "|"

        for estados_finais in estados_aceitacao:
           indices = [i for i, elem in enumerate(self.result[estado_inicial]) if estados_finais in elem]
           for index in indices:
               tmp += '|' + self.result[estado_inicial][index-1]
