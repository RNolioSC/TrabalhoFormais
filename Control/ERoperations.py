...
class ERoperations:
    ...
    def er_to_af(er_expression):
        ...
        # Criar tabela
        ERoperations.criar_tabela(arvore)
    ...
    def nodo_raiz(arvore, new_er_expression):
        ...
        nodo_raiz = Nodo(operador_atual, None, None, True, False)
    ...
    @staticmethod
    def criar_tabela(elemento, direcao):
        if elemento.get_eh_folha():
            pass

        if elemento.get_simbolo() == '*':
            ERoperations.criar_tabela(elemento.get_filho_esquerda(), 'DESCER')
            ERoperations.criar_tabela(elemento.get_costura(), 'SUBIR')
        if elemento.get_simbolo() == '.':
            if direcao == 'DESCER':
                ERoperations.criar_tabela(elemento.get_filho_esquerda(), 'DESCER')
            else:
